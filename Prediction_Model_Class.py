...
class Image_Processor(object):
    ...
    def post_process(self, images, annotation=None):
...
class Normalize_Images(Image_Processor):
    def __init__(self, mean_val=0, std_val=1, upper_threshold=300, lower_threshold=-100, max_val=255):
...
def run_model(gpu=0):
    ...
    with tf.device('/gpu:' + str(gpu)):
        ...
        model_info = {'model_path':os.path.join('C:\\users\\bmanderson\\Desktop\\','weights-improvement-best.hdf5'),
                      'names':['UterineCervix_BMA_Program_4'],'vgg_model':[], 'image_size':512,'three_channel':True,
                      'path':[#os.path.join(shared_drive_path,'Liver_Auto_Contour','Input_3'),
                              os.path.join(morfeus_path, 'Morfeus', 'Auto_Contour_Sites', 'Cervix_Auto_Contour','Input_3'),
                              os.path.join(raystation_drive_path,'Cervix_Auto_Contour','Input_3')],'is_CT':True,
                      'single_structure': True,'mean_val':80,'std_val':42,'vgg_normalize':False,'threshold':0.5,'file_loader':base_dicom_reader,
                      'post_process': partial(normalize_images, lower_threshold=-1000, upper_threshold=1000, is_CT=True,
                                              mean_val=80, std_val=42),
                      'pad':Image_Clipping_and_Padding(layers=3)}
        # models_info['liver_2'] = model_info
            ...
            for key in models_info.keys():
                ...
                with session1.as_default():
                    ...
                    models_info[key]['vgg_model'] = VGG_Model_Pretrained(models_info[key]['model_path'],num_classes=num_classes,gpu=gpu,
                                                                         image_size=models_info[key]['image_size'],graph1=graph1,session1=session1, Bilinear_model=BilinearUpsampling)
                    models_info[key]['predict_model'] = Predict_On_Models(models_info[key]['vgg_model'],
                                                                          is_CT=models_info[key]['is_CT'],vgg_normalize=models_info[key]['vgg_normalize'],
                                                                          image_size=models_info[key]['image_size'],three_channel=models_info[key]['three_channel'],
